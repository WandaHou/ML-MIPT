...
class LlamaPredictor(torch.nn.Module):
    ...
    def __init__(self, L_max, n_embd, n_layer, n_head, vocab_size, dropout_prob):
        super().__init__()
        self.L_max = L_max
    ...
    def mask(self, measure):
        return (measure != 0).to(torch.int32).to(device=measure.device)
        
    def forward(self, measure):
        attention_mask = self.mask(measure)
